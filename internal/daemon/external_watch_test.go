...
func TestExternalWatch_UnattributedChangeFlagsAfterGrace(t *testing.T) {
	...
	w.Observe(ctx, "A") // baseline
	w.Observe(ctx, "B") // first sighting: pending, not flagged yet
	if len(*notified) != 0 {
		t.Fatalf("flagged before grace elapsed")
	}
	w.Observe(ctx, "B") // second sighting: external
	if len(*notified) != 1 {
	...
	// attribution).
	w.Observe(ctx, "C")
	w.Observe(ctx, "C") // external #2 (audited; notification rate-limited)
	w.Observe(ctx, "B")
	w.Observe(ctx, "B") // would be wrongly "own" if external rows counted
...
func TestExternalWatch_AuditRowLandsDuringGrace(t *testing.T) {
	...
	w.Observe(ctx, "A")
	w.Observe(ctx, "B") // pending — audit row not written yet
	_ = s.InsertSwitchAudit(ctx, store.SwitchAuditRecord{
		Ts: *now, ToLabel: "B", Trigger: store.TriggerAutoswitch,
	})
	w.Observe(ctx, "B") // grace check finds the row → own
	if len(*notified) != 0 {
...
// Empty labels (transient resolution failures) are ignored entirely: no
// baseline reset, no change events in either direction.
func TestExternalWatch_EmptyLabelIgnored(t *testing.T) {
	w, _, notified, _ := watcherHarness(t)
	ctx := context.Background()

	w.Observe(ctx, "A")
	w.Observe(ctx, "") // hiccup
	w.Observe(ctx, "A")
	if len(*notified) != 0 {
...
func TestExternalWatch_NotifyCooldown(t *testing.T) {
	...
	w.Observe(ctx, "A")
	w.Observe(ctx, "B")
	w.Observe(ctx, "B") // external #1 → notify
	w.Observe(ctx, "C")
	w.Observe(ctx, "C") // external #2, inside cooldown → no notify
	if len(*notified) != 1 {
