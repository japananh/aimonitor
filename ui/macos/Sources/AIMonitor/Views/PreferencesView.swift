    ...
    private func tokenGuide(_ service: String)
        ...
        if service == "slack" {
            return (
                ...
                "Required User Token Scopes: search:read, users:read, users:read.email, channels:history, groups:history, im:history, mpim:history, channels:read, groups:read, im:read, mpim:read, chat:write, files:write"
            )
        }
        if service == "sentry" {
            return (
